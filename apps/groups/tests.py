...
class GroupTest(unittest.TestCase):
    ...
    def setUp(self):
        self.tearDown()
        self.c = Client()
        self.user_group_admin = User.create_user(email='group_admin@web-mark.ru', password='123')
        self.user_other = User.create_user(email='other@web-mark.ru', password='123')
        self.user_superuser = User.create_user(email='superuser@web-mark.ru', password='123', is_superuser=True)
    ...
    def test_group_create(self):
        self.c.login(email='group_admin@web-mark.ru', password='123')
    ...
    def test_free_group_view(self):
        ...
        self.c.login(email='other@web-mark.ru', password='123')
        response = self.c.get(reverse('groups:group_view', args=[group_id]))
        self.assertEqual(response.status_code, 200)

        self.c.login(email='superuser@web-mark.ru', password='123')
        response = self.c.get(reverse('groups:group_view', args=[group_id]))
        self.assertEqual(response.status_code, 200)
